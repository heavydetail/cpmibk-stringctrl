    ...
    while (myPort.available() > 0) {
      ...
      if (inBuffer != null) {
         ...
         if(tokens.length == 3 && tokens[0].equals("SETLIGHT")) {
           updateLightsbyID(PApplet.parseInt(trim(tokens[1])),PApplet.parseInt(trim(tokens[2]))); //INT conv.
...
public void updateLightsbyID(int _id, int _brightness) {
  int increment = 20;
...
public void updateLightsGroup(int id, int _brightness){
  int increment = 100;
